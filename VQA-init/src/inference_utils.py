...
from transformers import Blip2Processor, GPT2Tokenizer, T5Tokenizer, BlipImageProcessor
...
finetune_folder = '/root/autodl-tmp/models/blip2-vizwizqa'
finetune_folder = os.path.join('/root/autodl-tmp/models/blip2-vizwizqa')
...
def is_single_groundings(predicted_grounding_masks: List[np.ndarray], threshold=0.2, is_common_answer=None): 
    """
    判断是否为单目标
    
    Args:
        predicted_grounding_masks: 预测的掩码列表
        threshold: IoU阈值
        is_common_answer: 标记哪些答案是通用答案的布尔列表
    """
    if len(predicted_grounding_masks) == 0:
        return False
        
    # 如果只有一个掩码，直接判断为单目标
    if len(predicted_grounding_masks) == 1:
        return True
        
    # 如果提供了通用答案标记
    if is_common_answer is not None:
        # 过滤掉通用答案对应的掩码
        real_masks = [mask for i, mask in enumerate(predicted_grounding_masks) if not is_common_answer[i]]
        ...
        # 如果过滤后有多个掩码，只比较这些掩码
        if len(real_masks) > 1:
            reference_mask = real_masks[0]
            for mask in real_masks[1:]:
                if mask.shape != reference_mask.shape:
                    return False
                intersection = np.logical_and(mask, reference_mask)
                union = np.logical_or(mask, reference_mask)
...
# def inference_blip(input_image, input_text, threshold=0.3, print_preds=True): # 原置信度
...
#     processor = Blip2Processor.from_pretrained(finetune_folder)
#     tokenizer = GPT2Tokenizer.from_pretrained(finetune_folder)
#     model = Blip2ForConditionalGeneration.from_pretrained(finetune_folder).to('cuda')
